...
// ---------------------------------写方法区-----------------------
// 素材获取
function material_gain(materialTag,appid,appid_key,used_times_model,used_counter) {
    try {
        log("正在素材获取")
        var material_gain = httpUtilFunc.materialGet({
            ...
            "count": 1, // 取数量 默认1个
            "type": 0, // 素材类型（0:纯文本；1:图片；2:视频；3:音频）
            "classify": 3,  // 文本类型(0:默认；1:昵称；2:简介；3:外链; 4:对话模板) / 图片类型(0:默认；1:发布内容；2:头像) / 视频类型(0:默认；1:发布内容)
