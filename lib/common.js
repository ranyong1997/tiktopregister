...
commonFunc.init = function () {
    ...
    try {
        ...
        // toastLog(shell("getprop gsm.serial"))
    } catch (error) { }
    //  获取用户id
    try {
        commonFunc.userId = -1
...
commonFunc.facebookInstall = function (url) {
    if (!app.getAppName("com.facebook.katana")) {
        if (!httpUtilFunc.downloadFile(url, "/storage/emulated/0/Facebook_v318", 1000 * 60, false) || !commonFunc.installApk("/storage/emulated/0/Facebook_v318", 1000 * 60)) {
            return false
        }
    }
    return true
