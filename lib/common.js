...
commonFunc.taskResultSet = function (result, mode) {
    try {
        let filepath = "/storage/emulated/obb/logs/" + commonFunc.taskid + ".log"
        if (result != null) {
            if (!files.exists(filepath)) { files.createWithDirs(filepath) }
            let result_str = commonFunc.objectToString(result)
            log(result_str)
            try { result_str = result_str.replace(new RegExp(/[^\w\u4e00-\u9fa5\r\n\s`~!@#$%^&*()_\-+=<>?:"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘'，。、]/img), "") } catch (error) { }
            files.write(filepath, result_str + " \n")
            return true
        }
