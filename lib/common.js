...
commonFunc.init = function () {
    log("正在初始化【commonFunc】")
    // 如果fliderId不为空，则返回
    if (commonFunc.folderId != null) {
        return
    }
    try {
        commonFunc.taskid = ""
...
commonFunc.getAppVersionName = function (bid) {
    log("当前版本号:" + context.getPackageManager().getPackageInfo(bid, 0).versionName)
    try {
...
commonFunc.uninstallApp = function (bid) {
    try {
        ...
        log("卸载app时发现一个错误: " + JSON.stringify(error))
    }
...
commonFunc.tiktokInstall = function (url) {
    if (!app.getAppName("com.ss.android.ugc.trill")) {
        if (!httpUtilFunc.downloadFile(url, "/storage/emulated/0/Tiktok_v19.2.4", 1000 * 60, false) || !commonFunc.installApk("/storage/emulated/0/Tiktok_v19.2.4", 1000 * 60)) {
