...
proxySttings.kitsunebiInstall = function (url) {
    if (!app.getAppName("fun.kitsunebi.kitsunebi4android")) {
        if (!httpUtilFunc.downloadFile(url, "/storage/emulated/obb/fun.kitsunebi.kitsunebi4android_83.apk", 1000 * 60, false) || !commonFunc.installApk("/storage/emulated/obb/fun.kitsunebi.kitsunebi4android_83.apk", 1000 * 60)) {
            return false
        }
    }
    return true
...
proxySttings.kitsunebiSetup_old = function (proxy_info, proxy_provider, force_update) {
    let server_name = null
    let server_address = null
    let server_port = null
    let server_type = null
                                ...
                                if (latency) {
                                    ...
                                    if (proxy_provider == "liao_proxy" && clickIfWidgetExists(text(server_name).findOne(1000))) {
                                        try {
                                            ...
                                            // log( "switch-ip?cityid=68000: " + commonFunc.objectToString(res) )
                                            if (res.code != 0) { throw res }
                                        } catch (error) { err_msg = "switch-ip?cityid=68000 error: " + commonFunc.objectToString(res) }
                                        sleep(10000)
                                    }
                                }
                                try { toastLog("节点测试: " + id("fun.kitsunebi.kitsunebi4android:id/ep_latency").findOne(1000).text()) } catch (error) { }
